...
class GeminiStandardizer:
    """
    High-throughput batch standardizer using Google Gemini 3.5 Flash-Lite
    ...
    def _batch_values(self, values: List[str], task_type: str) -> List[List[str]]:
        ...
        max_items_per_batch = 25  # Keep LLM output highly accurate and prevent array length mismatch
    ...
    def _strip_thinking_and_extract_json(self, text: str) -> str:
        """Remove reasoning / thinking artifacts and extract JSON array."""
        if not text:
            return "[]"
    ...
    def _call_with_retry(self, prompt: str, max_tokens: int) -> str:
        ...
        while retry_count < self.max_retries:
            try:
                ...
                err_str = str(e).lower()
                if "403" in err_str or "permission" in err_str or "api_key" in err_str:
                    print(f"[Error] Gemini API Key/Auth issue: {e}")
                    raise
                elif any(x in err_str for x in ["429", "rate", "resource_exhausted", "resourceexhausted", "quota", "timeout", "503"]):
                    jitter = random.uniform(0.8, 1.2)
                    wait_time = min(delay * (2 ** retry_count) * jitter, 60.0)
    ...
    def _batch_call(self, values: List[str], task_type: str, instruction_template: str) -> List[Any]:
        """Main batched Gemini call with caching."""
        if not values:
            return []
        ...
        if uncached_unique:
            ...
            for batch_idx, batch in enumerate(batches):
                if batch_idx > 0:
                    time.sleep(1.2)  # Pace batch calls to stay smoothly under free-tier RPM limits
                ...
                try:
                    ...
                    if parsed is not None and len(parsed) == len(batch):
                        ...
                        # Fallback: use raw value if length mismatch
    ...
    def standardize_vendors_batch(self, vendors: List[str]) -> List[str]:
        """
        Batch standardize vendor names or bank descriptions using Gemini 3.5 Flash-Lite.
    ...
    def standardize_descriptions_batch(self, descriptions: List[str]) -> List[str]:
        """
        Summarize ALL descriptions in ONE batched API call using Gemini 3.5 Flash-Lite.
