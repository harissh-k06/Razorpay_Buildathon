...
const categoryColors: Record<FaqItem["category"], string> = {
  account: "bg-[#305EFF]/10 text-[#305EFF]",
  payments: "bg-[#009E5C]/10 text-[#009E5C]",
...
const ticketStatusConfig: Record<SupportTicket["status"], { label: string; icon: React.ReactNode; className: string }> = {
  open: { label: "Open", icon: <CircleDotIcon className="size-3" />, className: "bg-[#305EFF]/10 text-[#305EFF] border-[#305EFF]/20" },
  "in-progress": { label: "In Progress", icon: <ClockIcon className="size-3" />, className: "bg-amber-500/10 text-amber-600 border-amber-500/20" },
  resolved: { label: "Resolved", icon: <CheckCircle2Icon className="size-3" />, className: "bg-[#009E5C]/10 text-[#009E5C] border-[#009E5C]/20" },
}
...
function ContactChannels() {
  return (
    <div className="grid gap-3 sm:grid-cols-3">
      {[
        { icon: <MessageCircleIcon className="size-5" />, label: "Live Chat", desc: "Avg. wait: 2 min", badge: "Online", badgeColor: "bg-[#009E5C]", color: "bg-[#009E5C]/10 text-[#009E5C]" },
        { icon: <MailIcon className="size-5" />, label: "Email Support", desc: "Response within 24h", badge: null, badgeColor: "", color: "bg-[#305EFF]/10 text-[#305EFF]" },
        { icon: <HeadphonesIcon className="size-5" />, label: "Phone", desc: "Mon-Fri, 9am-6pm", badge: null, badgeColor: "", color: "bg-purple-500/10 text-purple-600" },
      ].map((ch) => (
        ...
        <CardContent className="flex items-center gap-4 p-6">
          <motion.div
            ...
            className={cn(
              ...
              allOperational ? "bg-emerald-500/10 text-emerald-600" : "bg-amber-500/10 text-amber-600"
            )}
          >
